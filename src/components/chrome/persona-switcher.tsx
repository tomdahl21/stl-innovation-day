...
export function PersonaSwitcher() {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((o) => !o)}
        aria-label={`Switch persona — currently ${active?.displayName ?? "unknown"}`}
        aria-expanded={open}
        aria-haspopup="menu"
      ...
      {open && (
        <>
          ...
          <div
            role="menu"
            aria-label="Persona and navigation"
            className="absolute right-0 top-9 z-50 w-64 overflow-hidden rounded-xl bg-surface shadow-[0_16px_36px_-12px_rgba(26,22,18,0.35)] ring-1 ring-stone-line/60"
          >
            ...
            {personas.map((p) => (
              ...
              >
                <span
                  aria-hidden="true"
                  className="flex h-7 w-7 items-center justify-center rounded-full bg-moss text-[11px] font-medium text-paper"
                >
                  {p.initials}
                </span>
                <span className="flex-1 min-w-0">
                  ...
                  <span className="block truncate text-[11px] uppercase tracking-[0.1em] text-ink-muted">
                    {p.tier === "founding"
                      ? "Founding member"
                      : p.tier === "verified"
                        ? "Verified local"
            ...
            >
              <span
                aria-hidden="true"
                className="flex h-7 w-7 items-center justify-center rounded-full bg-paper-warm text-[12px] text-ink-soft"
              >
                →
              </span>
            ...
            >
              <span
                aria-hidden="true"
                className="flex h-7 w-7 items-center justify-center rounded-full bg-paper-warm text-[12px] text-brick"
              >
                ✶
              </span>
